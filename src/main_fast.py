...
def crawl(apk,websites,run_no):
    ...
    for url in websites:
        version = browser + '_run_' + str(run_no)
        frag = '#'
        #if not url.endswith('/'):
        frag  = '/' + frag
        _url = url + frag + version + 'simaliarVersions_ON_A7'
        print(_url)
        for i in range(4):
            adb.tap_ok()
        adb.launch_url_video(pkgId,launAct,launMethod,apkName,str(run_no),True,_url)
        time.sleep(15)
        ...
        for run_no in range(2,3):
            for apk in _apks:
                ...
                if browser.startswith('Op') and run_no == 2:
                        continue
                if browser.startswith('Ma') and run_no == 2 :
                        continue
                if browser.startswith('Tu') and run_no ==1:
                        continue
                if browser.startswith('Op') and run_no ==2:
                        continue
                ...
                if bypaRes:
                    ...
                    if browser.startswith('Chrome-63') and run_no ==2:
                        _websites = websites[89:]
                    else:
                        _websites = websites
                    #TEMP
