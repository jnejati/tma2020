...
def crawl(apk,websites,run_no):
    ...
    for url in websites:
        for _run in range(run_no):
            ...
            for i in range(4):
                adb.tap_ok()
            adb.launch_url_video(pkgId,launAct,launMethod,apkName,str(_run),True,_url)
            time.sleep(5)
    ...
    with open('../logs/apkslog.csv', 'w') as apks_log:
        for apk in _apks:
            ...
            if bypaRes:
                ...
                j=0
                for i in range(0, len(websites) - 5, 5):
                    #TEMP
                    print('browser:', browser)
                    """if browser.startswith('Ex') and i < 5:
                        j=i+5
                        continue"""
                    #TEMP
                    _websites = websites[j:i+5]
                ...
                time.sleep(4000)
                """apkinfo = apk + [launAct,launMethod]
                #for _files in os.listdirs('../config/testsuites/'):
                #_file = os.path.join('/home/jnejati/bperf/config/testsutes', _files)
                #websites = [item[0] for item in read_data(_file)]
                websites = [item[0] for item in read_data("../config/testsuite.txt")]
